  ...
  context "languages that should be passed through" do
    ...
    %w(math mermaid plantuml suggestion).each do |lang|
      ...
      context "when #{lang} has extra params" do
        ...
        it "includes lang-params tag with extra information" do
          result = filter(%{<pre><code lang="#{lang}#{delimiter}#{lang_params}">This is a test</code></pre>})

          expect(result.to_html).to eq(%{<pre class="code highlight js-syntax-highlight #{lang}" lang="#{lang}" lang-params="#{lang_params}" v-pre="true"><code><span id="LC1" class="line" lang="#{lang}">This is a test</span></code></pre>})
        end
    ...
    context 'when multiple param delimiters are used' do
      ...
      it "delimits on the first appearence" do
        ...
        expect(result.to_html).to eq(%{<pre class="code highlight js-syntax-highlight #{lang}" lang="#{lang}" lang-params="#{lang_params}#{delimiter}more-things" v-pre="true"><code><span id="LC1" class="line" lang="#{lang}">This is a test</span></code></pre>})
      end
    end
  end
