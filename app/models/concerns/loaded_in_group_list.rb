...
module LoadedInGroupList
  ...
  def project_count
    @project_count ||= try(:preloaded_project_count) || projects.non_archived.size
  end

  def subgroup_count
    @subgroup_count ||= try(:preloaded_subgroup_count) || children.size
  end

  def member_count
    @member_count ||= try(:preloaded_member_count) || users.size
  end
end
