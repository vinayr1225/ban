...
module Suggestions
  class CreateService
    ...
    def execute
      ...
      # TODO: Make use of the parsed above/below context and persist the correct
      # lines.
      suggestions = Banzai::SuggestionsParser.parse(@note.note, project: @note.project,
                                                                position: @note.position)

      rows =
        suggestions.map.with_index do |suggestion, index|
          creation_params = suggestion.to_hash.slice(:from_content, :to_content)
